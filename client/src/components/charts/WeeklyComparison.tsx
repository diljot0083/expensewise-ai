...
const WeeklyComparison = ({ expenses }: { expenses: Expense[] }) => {
    ...
    const percent =
        lastWeek === 0 ? 100 : Math.abs(Number(((diff / lastWeek) * 100).toFixed(1)));
    ...
    return (
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-4 rounded-2xl">
            ...
            <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500 dark:text-gray-400">Last week</span>
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-100 tabular-nums">
                        ₹{lastWeek}
                    </span>
                </div>
                ...
                <div
                    className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-lg ${isUp
